...
% \setlist[enumerate]{noitemsep,topsep=4pt,label=\arabic*.}
  ...
  You are strongly encouraged to work on this project in groups of up to
  three. Each member of the group must submit an (identical) copy of the
  project before the deadline 23:59 22.3.2026 on Inspera.
  
  Each submission should clearly indicate which points of the project
  each member of the group contributed to. It is acceptable for all
  members to contribute to all points. \emph{For full marks, all
    numbered points should be addressed.}

  The project should be completed as a written report, including a
  typeset \verb|pdf| file. The coding component must be runnable. It is
  recommended to submit your code as a Jupyter notebook, with minimal
  external dependencies. You should submit the pdf file and code as a
  single \verb|zip| file. The clarity of your report counts towards the final
  grade.


  Please direct questions about the project towards Anja, who can be
  reached via email at \texttt{anjarin@stud.ntnu.no}.
  ...
  technical components. For example, in order to avoid hospital spacial
  ineciency, it is important to learn how to optimize the spaces such
  ...
  to nd the best place to locate this warehouse in order to minimize
  costs of travel and transportations. Thus, location problems appear in
  many variants and with dierent constraints depending on the practical
...
\begin{equation}\label{P}
  ...
  Before presenting an algorithm for solving problem ( \(P_{d_{2}}\) ), we give the following result.\\
  Theorem 1 (see: Love, Morris Wesolowsky, 1988, Property 2.2). If for
...
\item If \(x^{\text {new }}=\left(x_{1}^{\text {new }}, x_{2}^{\text {new }}\right)\) satisfies a stopping criterion w.r.t. the given \(\epsilon\) (see \S\ref{sec:terminate} below) then set \(x^{*}=x^{\text {new }}\). Otherwise: Set \(x=x^{\text {new }}\) and go to Step 3.
...
\[
  \begin{aligned}
    f_{d_{2}}(x)-f_{d_{2}}\left(x^{*}\right) & \leq\left\langle\nabla f_{d_{2}}(x), x-x^{*}\right\rangle \\
                                             & \text { Cauchy-Schwartz } \underset{\leq}{\leq}\left\|\nabla f_{d_{2}}(x)\right\| \cdot\left\|x-x^{*}\right\| \\
                                             & \leq\left\|\nabla f_{d_{2}}(x)\right\| \cdot \sigma(x)
  \end{aligned}
\]
\end{proof}
...
\begin{theorem} \label{thm3}
  Let \(x^{*}\) denote the global minimizer of \(\left(P_{d_{2}}\right)\). A lower bound for \(f_{d_{2}}\left(x^{*}\right)\) is given by
  \[
...
\begin{theorem} \label{thm4}
  ...
  \item Implement the Weiszfeld algorithm using Python, including a
    test example. Your code must be runnable.
  \item Implement the gradient descent method with backtracking and
    replace steps (b)-(d) in the above algorithm with it. Compare with
    the Weiszfeld algorithm and discuss the difference between the two
